...
describe("PATCH /api/user/password", () => {
  ...
  it("retorna 400 quando usuário não tem senha (Google OAuth)", async () => {
    ...
    const { PATCH } = await import("./route")
    const res = await PATCH(makeRequest({ currentPassword: "old", newPassword: "Secure!Pass1" }))
    expect(res.status).toBe(400)
  ...
  it("retorna 400 para senha atual incorreta", async () => {
    ...
    const { PATCH } = await import("./route")
    const res = await PATCH(makeRequest({ currentPassword: "wrong", newPassword: "Secure!Pass1" }))
    expect(res.status).toBe(400)
    const data = await res.json()
  ...
  it("altera senha com sucesso", async () => {
    ...
    mockSelect.mockReturnValue([{ password: "hashed-old" }])
    const { PATCH } = await import("./route")
    const res = await PATCH(makeRequest({ currentPassword: "correct", newPassword: "Secure!Pass1" }))
